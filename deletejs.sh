...
if [ -f "$HOME/.Yunzai" ]; then
  echo -e "\033[35m校验成功\033[0m"
else
  echo -e "\033[35m初始化文件中\033[0m"
  sleep 0.3
...
echo -e "\033[1;35m已安装的js插件：\033[0m"
echo
# 读取目录中的文件，并为每个文件标上序号
files=(plugins/example/*)
num=1
for file in "${files[@]}"; do
    echo -e "\e[35m$num.\e[0m \e[35m$(basename "$file")\e[0m"
    ((num++))
done
...
if [[ $confirmation =~ ^([yY]|[yY][eE][sS])$ ]]; then
    ...
    # 输出删除成功信息
    echo -en "\033[35m 卸载完成 回车返回\033[0m";read -p ""
else
